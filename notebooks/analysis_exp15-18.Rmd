---
title: "Exp analysis - exp15-18 - ING - g/stim_rate"
...
path <- "~/Code/theagamma/data/exp15/"
dmi_exp15 <- NULL
for(g in gs){
  for(stim_rate in stim_rates){
    ...
    dmi_exp15 <- bind_rows(df, dmi_exp15)
  }
}
...
for(g in gs){
  for(stim_rate in stim_rates){
    ...
    df$g <- as.numeric(g)
    df$stim_rate <- as.numeric(stim_rate)
    pow_exp15 <- bind_rows(df, pow_exp15)
  }
}
...
## Power v dMI
```{r, fig.width=4, fig.height=2, echo=FALSE, warning=FALSE, message=FALSE}
tmp <- right_join(pow_exp15, dmi_exp15)
tmp %>% 
