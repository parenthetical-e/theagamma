...
path <- "~/Code/theagamma/data/exp7/"
dmi_exp7 <- NULL
for(g in gs){
  file_name <- paste(path, "result", as.character(g), "_dMI.csv", sep="")
  df <- read.csv(file_name)
  df$g <- g
  dmi_exp7 <- bind_rows(df, dmi_exp7)
}
...
for(g in gs){
  ...
  df <- read.csv(file_name)
  df$g <- g
  pow_exp7 <- bind_rows(df, pow_exp7)
}

# exp8
path <- "~/Code/theagamma/data/exp8/"
dmi_exp8 <- NULL
for(g in gs){
  for (num_pop in num_pops) {
    ...
    df <- read.csv(file_name)
    df$g <- g
    dmi_exp8 <- bind_rows(df, dmi_exp8)
  }
}
```
...
## Power v dMI
```{r, fig.width=4, fig.height=2, echo=FALSE, warning=FALSE, message=FALSE}
tmp <- right_join(pow_exp7, dmi_exp7)
tmp %>% 
