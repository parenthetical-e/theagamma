---
title: "Exp analysis - exp21-22 - PING - g_ie/stim_rate"
...
path <- "~/Code/theagamma/data/exp21/"
dmi_exp21 <- NULL
for(g in gs){
  for(stim_rate in stim_rates){
    ...
    dmi_exp21 <- bind_rows(df, dmi_exp21)
  }
}
...
for(g in gs){
  for(stim_rate in stim_rates){
    ...
    df$g <- as.numeric(g)
    df$stim_rate <- as.numeric(stim_rate)
    pow_exp21 <- bind_rows(df, pow_exp21)
  }
}
...
## Power v dMI
```{r, fig.width=4, fig.height=2, echo=FALSE, warnPING=FALSE, message=FALSE}
tmp <- right_join(pow_exp21, dmi_exp21)
tmp %>% 
