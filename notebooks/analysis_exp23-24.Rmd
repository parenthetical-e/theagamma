---
title: "Exp analysis - exp23-24 - CHING - g_ie/stim_rate"
...
path <- "~/Code/theagamma/data/exp23/"
dmi_exp23 <- NULL
for(g in gs){
  for(stim_rate in stim_rates){
    ...
    dmi_exp23 <- bind_rows(df, dmi_exp23)
  }
}
...
for(g in gs){
  for(stim_rate in stim_rates){
    ...
    df$g <- as.numeric(g)
    df$stim_rate <- as.numeric(stim_rate)
    pow_exp23 <- bind_rows(df, pow_exp23)
  }
}
...
## Stim (color g)
### Boxplot
```{r, fig.width=6, fig.height=8, echo=FALSE, warning=FALSE, message=FALSE}
dmi_exp23 %>% 
  gather(model, mi, stim_p:osc) -> tmp
...
dmi_exp23 %>% 
  group_by(stim_rate, g) %>% 
  summarise(M=mean(I), SD=sd(I)) -> tmp
...
## Power v dMI
```{r, fig.width=4, fig.height=2, echo=FALSE, warning=FALSE, message=FALSE}
tmp <- right_join(pow_exp23, dmi_exp23)
tmp %>% 
...
# Exp 18 (pop size)
## dMI 
```{r, fig.width=6, fig.height=12, echo=FALSE, warning=FALSE, message=FALSE}
dmi_exp24 %>% 
  filter(num_pop >= 40) %>% 
