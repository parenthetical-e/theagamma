---
title: "Exp analysis - exp11-12 - PING/g_ie"
...
path <- "~/Code/theagamma/data/exp11/"
dmi_exp11 <- NULL
for(g in gs){
  file_name <- paste(path, "result", as.character(g), "_dMI.csv", sep="")
  df <- read.csv(file_name)
  df$g <- g
  dmi_exp11 <- bind_rows(df, dmi_exp11)
}
...
for(g in gs){
  ...
  df <- read.csv(file_name)
  df$g <- g
  pow_exp11 <- bind_rows(df, pow_exp11)
}

# exp12
path <- "~/Code/theagamma/data/exp12/"
dmi_exp12 <- NULL
for(g in gs){
  for (num_pop in num_pops) {
    ...
    df <- read.csv(file_name)
    df$g <- g
    dmi_exp12 <- bind_rows(df, dmi_exp12)
  }
}
```
...
## Power v dMI
```{r, fig.width=4, fig.height=2, echo=FALSE, warning=FALSE, message=FALSE}
tmp <- right_join(pow_exp11, dmi_exp11)
tmp %>% 
