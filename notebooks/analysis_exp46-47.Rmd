---
title: "Exp analysis - exp46-47 - PING - tau_e/stim_rate"
...
path <- "~/Code/theagamma/data/exp46/"
dmi_exp46 <- NULL
for(g in taus){
  for(stim_rate in stim_rates){
    ...
    dmi_exp46 <- bind_rows(df, dmi_exp46)
  }
}
...
for(g in taus){
  for(stim_rate in stim_rates){
    ...
    df$g <- as.numeric(g)
    df$stim_rate <- as.numeric(stim_rate)
    pow_exp46 <- bind_rows(df, pow_exp46)
  }
}
...
## Power v dMI
```{r, fig.width=5, fig.height=5, echo=FALSE, warnPING=FALSE, message=FALSE}
tmp <- right_join(pow_exp46, dmi_exp46)
tmp %>% 
  ggplot(aes(x=lfp_gamma, y=E, color=as.factor(g))) +
  geom_point(size=1) +
...
dmi_exp47 %>% 
  filter(num_pop >= 40) %>% 
  filter(stim_rate == 1.5) %>% 
  ggplot(aes(x=g, y=E)) +
  geom_jitter() +
  geom_hline(yintercept=0, color="grey") + xlab("") +
  labs(y = expression(paste(Delta, "MI (bits)")),
       x = expression(tau[E]),
       title = "Model: PING (E to I)") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  theme(legend.background = element_rect(colour ="black")) +
  theme(legend.key.size = unit(.3, "cm")) +
