---
title: "Exp analysis - exp44-45 - ING - tau/stim_rate"
...
path <- "~/Code/theagamma/data/exp44/"
dmi_exp44 <- NULL
for(g in taus){
  for(stim_rate in stim_rates){
    ...
    dmi_exp44 <- bind_rows(df, dmi_exp44)
  }
}
...
for(g in taus){
  for(stim_rate in stim_rates){
    ...
    df$g <- as.numeric(g)
    df$stim_rate <- as.numeric(stim_rate)
    pow_exp44 <- bind_rows(df, pow_exp44)
  }
}
...
## Power v dMI
```{r, fig.width=4, fig.height=2, echo=FALSE, warning=FALSE, message=FALSE}
tmp <- right_join(pow_exp44, dmi_exp44)
tmp %>% 
