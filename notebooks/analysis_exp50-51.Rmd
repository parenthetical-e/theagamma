---
title: "Exp analysis - exp50-51 - CHING - tau_e/stim_rate"
...
path <- "~/Code/theagamma/data/exp50/"
dmi_exp50 <- NULL
for(g in taus){
  for(stim_rate in stim_rates){
    ...
    dmi_exp50 <- bind_rows(df, dmi_exp50)
  }
}
...
for(g in taus){
  for(stim_rate in stim_rates){
    ...
    df$g <- as.numeric(g)
    df$stim_rate <- as.numeric(stim_rate)
    pow_exp50 <- bind_rows(df, pow_exp50)
  }
}
...
## Power v dMI
```{r, fig.width=4, fig.height=2, echo=FALSE, warning=FALSE, message=FALSE}
tmp <- right_join(pow_exp50, dmi_exp50)
tmp %>% 
