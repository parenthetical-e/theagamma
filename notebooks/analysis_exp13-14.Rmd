---
title: "Exp analysis - exp13-14 - CHING/g_e"
...
path <- "~/Code/theagamma/data/exp13/"
dmi_exp13 <- NULL
for(g in gs){
  file_name <- paste(path, "result", as.character(g), "_dMI.csv", sep="")
  df <- read.csv(file_name)
  df$g <- g
  dmi_exp13 <- bind_rows(df, dmi_exp13)
}
...
for(g in gs){
  ...
  df <- read.csv(file_name)
  df$g <- g
  pow_exp13 <- bind_rows(df, pow_exp13)
}

# exp14
path <- "~/Code/theagamma/data/exp14/"
dmi_exp14 <- NULL
for(g in gs){
  for (num_pop in num_pops) {
    ...
    df <- read.csv(file_name)
    df$g <- g
    dmi_exp14 <- bind_rows(df, dmi_exp14)
  }
}
```
...
## Power v dMI
```{r, fig.width=4, fig.height=2, echo=FALSE, warning=FALSE, message=FALSE}
tmp <- right_join(pow_exp13, dmi_exp13)
tmp %>% 
