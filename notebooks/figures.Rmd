...
mi_exp15 <- load.data1(exp_path, "MI (bits)", stim_rates, qs, num_pops, qs)
...
mi_exp18 <- load.data7(exp_path, "MI (bits)", stim_rates, qs, num_pops, osc_rates, qs)
...
gs <- c(1, 2, 3, 4, 5, 6, 7, 8)
num_pops <- c(10240)
osc_rates <- c(2, 4, 6, 8, 10, 12, 14, 16, 18)
qs <- c("0.05", "0.25", "0.5", "0.75", "1.0")
exp_path <- paste(path, "/exp19/", sep="")
mi_exp19 <- load.data7(exp_path, "MI (bits)", stim_rates, qs, num_pops, osc_rates, qs)
...
# stim_rate / osc_rate
stim_rates <- c(5, 10, 15)
gs <- c(1, 2, 3, 4, 5, 6, 7, 8)
num_pops <- c(10240)
osc_rates <- c(2, 4, 6, 8)
qs <- c("0.05", "0.25", "0.5", "0.75", "1.0")
exp_path <- paste(path, "/exp20/", sep="")
mi_exp20 <- load.data7(exp_path, "MI (bits)", stim_rates, qs, num_pops, osc_rates, qs)
...
mi_exp21 <- load.data7(exp_path, "MI (bits)", stim_rates, qs, num_pops, osc_rates, qs)
...
mi_exp22 <- load.data7(exp_path, "MI (bits)", stim_rates, qs, num_pops, osc_rates, qs)
...
mi_exp23 <- load.data7(exp_path, "MI (bits)", stim_rates, qs, num_pops, osc_rates, qs)
...
mi_exp9 <- load.data2(exp_path, "MI (bits)", fs)
...
mi_exp10 <- load.data3(exp_path, "MI (bits)", osc_rates)
...
mi_exp11 <- load.data4(exp_path, "MI (bits)", ms)
...
mi_exp12 <- load.data5(exp_path, "MI (bits)", num_backgrounds)
...
mi_exp6 <- load.data6(exp_path, "MI (bits)", num_pops)
...
p1
ggsave("figures/arithmetic.pdf", dpi=300)
ggsave("figures/arithmetic.png", dpi=300)
```
# Figure - Examples
...
examples %>% 
  ...
  labs(x="Time (s)", y="Norm. rate", tag="a.", title="Discretize into m bins") +
  theme_classic2() + 
...
ggsave("figures/entropy.pdf", dpi=300)
ggsave("figures/entropy.png", dpi=300)
```
...
ggsave("figures/dist.pdf", dpi=300)
ggsave("figures/dist.png", dpi=300)
```
...
p1 + p2 + plot_layout(widths = c(0.15, 0.85)) +
  plot_annotation(tag_levels = 'a', tag_suffix = ".")
ggsave("figures/overall1.pdf", dpi=300)
ggsave("figures/overall1.png", dpi=300)
```
...
tmp %>% 
  ...
  labs(y=expression(paste(Delta, "MI (bits)")), x="Stimulus firing rate (spks/s)") +
...
tmp %>% 
  ...
  labs(y=expression(paste(Delta, "MI (bits)")), x="Pacemaker firing rate (spks/s)") +
  scale_color_manual("g", values = c(
    "lightsteelblue3",
    "lightskyblue3",
    "steelblue3",
...
ggsave("figures/overall2.pdf", dpi=300)
ggsave("figures/overall2.png", dpi=300)
```
...
((p1 / p3) | (p2 / p4)) + 
  ...
  plot_annotation(tag_levels = 'a', tag_suffix = ".", 
                  ...
                  theme=theme(plot.title = element_text(size = 11, face = "bold")))
ggsave("figures/overall3.pdf", dpi=300)
ggsave("figures/overall3.png", dpi=300)
```
...
tmp %>% 
  ...
  labs(y=expression(paste(Delta, "MI (bits)")), x="Stimulus firing rate (spks/s)", title = "Division fraction") +
  scale_color_manual("g", values = c(
    "lightsteelblue3",
    "lightskyblue3",
    "steelblue3",
...
p1 + p2 + plot_layout(widths = c(0.07, 0.9)) +
  plot_annotation(tag_levels = 'a', tag_suffix = ".", 
                  title="Population size: 40",
                  theme=theme(plot.title = element_text(size = 11, face = "bold")))
ggsave("figures/overall4.pdf", dpi=300)
ggsave("figures/overall4.png", dpi=300)
```
...
tmp %>% 
  ...
  labs(y=expression(paste(Delta, "MI (bits)")), x="Pacemaker frequency (Hz)") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  theme(strip.text.x = element_text(face = "bold")) +
...
tmp %>% 
  ...
  stat_summary(aes(group=num_background), fun=mean, 
               geom="point", colour="black", alpha=1, size=6, shape="_") +
  lims(y=c(-0.3, 0.5)) +
  labs(y=expression(paste(Delta, "MI (bits)")), x="Background size") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  theme(strip.text.x = element_text(face = "bold")) +
  facet_wrap(model~., nrow=1) ->
...
tmp %>% 
  ...
  stat_summary(aes(group=m), fun=mean, 
               geom="point", colour="black", alpha=1, size=6, shape="_") +
  lims(y=c(-0.3, 0.5)) +
  labs(y=expression(paste(Delta, "MI (bits)")), x="Dist. bin size (m)") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  theme(strip.text.x = element_text(face = "bold")) +
  facet_wrap(model~., nrow=1) -> 
...
p1 / p4 / p5 + 
  plot_annotation(tag_levels = 'a', tag_suffix = ".",
                  title="Population size: 40", 
                  theme=theme(plot.title = element_text(size = 11, face = "bold")))
ggsave("figures/control1.pdf", dpi=300)
ggsave("figures/control1.png", dpi=300)
```


# Figure - Power - small and large num_pop
- Kept all params. 
- See Supp. Figure as well.
```{r, fig.width=2.6, fig.height=2.6, echo=FALSE, warning=FALSE, message=FALSE}
dmi_exp23 %>% 
   gather(model, mi, back_p:div_sub_p) %>% 
  filter(model %in% c("add_p","mult_p", "sub_p", "div_sub_p")) -> tmp1

pow_exp23 %>% 
   gather(model, pow, back_p:div_sub_p) %>% 
  filter(model %in% c("add_p","mult_p", "sub_p", "div_sub_p")) -> tmp2
...
tmp %>% 
  # filter(osc_rate == 2, stim_rate == 10, g %in% c(2,3,4,5,6)) %>%
  # filter(g %in% c(4)) %>%
  ggplot(aes(x=pow, y=mi)) + 
  geom_point(size=.02, alpha=0.05) +
  stat_smooth(method="lm", se = TRUE, alpha=1, size=0.6, color="black") +
  lims(y=c(-0.5, 0.5)) +
  scale_x_continuous(breaks=c(0, 0.5, 1), limits=c(0, 1.25)) +
  labs(y=expression(paste(Delta, "MI (bits)")), x="Power", title="Population size: 40") +
  theme_classic() + 
  geom_hline(yintercept=0, color="dark grey", size=0.5) +
  theme(legend.background = element_rect(colour ="black")) +
  theme(strip.background = element_blank()) +
...
tmp %>% 
  ...
  geom_point(size=.02, alpha=0.05) +
  stat_smooth(method="lm", se = TRUE, alpha=1, size=0.6, color="black") +
  labs(y=expression(paste(Delta, "MI (bits)")), x="Power", title="Population size: 10240") +
  theme_classic() + 
  lims(y=c(-0.5, 0.5)) +
  scale_x_continuous(breaks=c(0, 0.5, 1), limits=c(0, 1.25)) +
  geom_hline(yintercept=0, color="dark grey", size=0.5) +
...
p1 / p2 + 
  plot_annotation(tag_levels = 'a', tag_suffix = ".")
ggsave("figures/power1.pdf", dpi=300)
ggsave("figures/power1.png", dpi=300)
```

# Supp. Figure - Power, facet by params
```{r, fig.width=5.2, fig.height=4.1, echo=FALSE, warning=FALSE, message=FALSE}
dmi_exp23 %>% 
   gather(model, mi, back_p:div_sub_p) %>% 
  filter(model %in% c("add_p","mult_p", "sub_p", "div_sub_p")) -> tmp1
...
tmp %>% 
  filter(model == "E") %>%
  ggplot(aes(x=pow, y=mi, color=factor(g))) +
  geom_point(size=.3, alpha=0.1) +
  stat_smooth(method="lm", se = FALSE, alpha=1) +
  geom_hline(yintercept=0, color="dark grey", size=0.5) + 
  labs(y=expression(paste(Delta, "MI (bits)")), x="Power", title="E") +
  scale_color_manual("g", values = c(
    "lightsteelblue3",
    "lightskyblue3",
    "steelblue3",
...
tmp %>% 
  ...
  stat_smooth(method="lm", se = FALSE, alpha=1) +
  geom_hline(yintercept=0, color="dark grey", size=0.5) + 
  labs(y=expression(paste(Delta, "MI (bits)")), x="Power", title="EI") +
  scale_color_manual("g", values = c(
    "lightsteelblue3",
    "lightskyblue3",
    "steelblue3",
...
tmp %>% 
  ...
  stat_smooth(method="lm", se = FALSE, alpha=1) +
  geom_hline(yintercept=0, color="dark grey", size=0.5) + 
  labs(y=expression(paste(Delta, "MI (bits)")), x="Power", title="I") +
  scale_color_manual("g", values = c(
    "lightsteelblue3",
    "lightskyblue3",
    "steelblue3",
...
tmp %>% 
  ...
  stat_smooth(method="lm", se = FALSE, alpha=1) +
  geom_hline(yintercept=0, color="dark grey", size=0.5) + 
  labs(y=expression(paste(Delta, "MI (bits)")), x="Power", title="II") +
  scale_x_continuous(breaks=c(0, 0.5, 1), limits=c(0, 1.25)) +
  scale_color_manual("g", values = c(
    "lightsteelblue3",
    "lightskyblue3",
...
(p1 + p2) / (p3 + p4) + 
  ...
  plot_annotation(title="Population size: 40", 
                  theme=theme(plot.title = element_text(size = 13, face = "bold")))
ggsave("figures/power2.pdf", dpi=300)
ggsave("figures/power2.png", dpi=300)
```
