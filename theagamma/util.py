...
class ProgressBar(object):
    ...
    def __call__(self, elapsed, complete, start, duration):
        if complete == 0.0:
            ...
            sys.stdout.write(
                "\b" *
                (self.toolbar_width + 1))  # return to start of line, after '['
        else:
...
def IrregularFluctuationPattern(v_base, v_new, TransitionTime, MinPlatoTime,
                                MaxPlatoTime, BaseTime, DT, T_simulation):
    ...
    Pattern = np.asarray([], dtype=float)
    STOP = 0
    #-----------------------------------------------------------------------------------
    while (STOP == 0):

        PlatoTime = np.random.uniform(low=MinPlatoTime,
                                      high=MaxPlatoTime,
                                      size=1)
        plato = np.full(int(PlatoTime / DT), v_new, dtype=float)

        Pattern_cicle = np.r_[np.r_[np.r_[plato_base, ramp_up], plato],
                              ramp_down]
        ...
        if (PatternLen >= (int(T_simulation / DT) - len(plato_base))):
            ...
            if (PatternLen > int(T_simulation / DT)):
                ...
                Pattern_final = np.r_[Pattern,
                                      np.full(int(T_simulation / DT) -
                                              len(Pattern),
                                              v_low,
                                              dtype=float)]
...
def Keiser_bandpass(data, time, lowcut, highcut, ripple_db, width, Nyquist,
                    SamplingRate):
    numtaps, beta = kaiserord(ripple_db, width / Nyquist)
    if (numtaps % 2 == 0):
        numtaps = (numtaps + 1)  #Numtaps must be odd
    taps = firwin(numtaps, [lowcut / Nyquist, highcut / Nyquist],
                  window=('kaiser', beta),
                  nyq=1.0,
                  pass_zero=False,
                  scale=True)
    ...
    filtered_data = filtered_data[numtaps -
                                  1:]  #taking out the "corrupted" signal
    time_filteredData = time[
        numtaps -
        1:] - delay  #correcting the delay and taking out the "corrupted" signal part
    data_thatOverLaps_FileteredOne = data[(numtaps - 1 - delay_index):(
        len(data) - (numtaps - 1 - delay_index))]
...
def SignalPhase_byHilbert(Signal, Signal_time, lowcut, highcut, SamplingSpace):
    ...
    time_Filtered_Signal, Filtered_Signal, Signal_thatOverLaps_FileteredOne = Keiser_bandpass(
        Signal, Signal_time, lowcut, highcut, ripple_db, width, Nyquist,
        SamplingRate)  #Use everything in seconds to obtain in seconds
    ...
    if (len(Filtered_Signal) % 2 != 0):  #If the lengh is odd
        ...
        tmp1.append(0)
        tmp2 = time_Filtered_Signal.tolist()
        tmp2.append((len(time_Filtered_Signal) + 1) * SamplingSpace +
                    time_Filtered_Signal[0])
        tmp3 = Signal_thatOverLaps_FileteredOne.tolist()
        tmp3.append(0)
        Filtered_Signal = np.asarray(tmp1)
        time_Filtered_Signal = np.asarray(tmp2)
        Signal_thatOverLaps_FileteredOne = np.asarray(tmp3)
    #--------------------------------------------------------------------------

    HT_Filtered_Signal = signal.hilbert(Filtered_Signal)
    Envelope = np.abs(HT_Filtered_Signal)
    Phase = np.angle(
        HT_Filtered_Signal)  #The phase is between -pi and pi in radians
